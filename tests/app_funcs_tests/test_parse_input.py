...
from core.app_classes import AmountTuple, QueryItem
...
def test_value_at_inx_0():
    r = parse_input(data=tup)
    a = AmountTuple(amount=1, quantity=1, unit="L")
...
def test_value_at_inx_1():
    r = parse_input(data=tup)
    a = AmountTuple(amount=100, quantity=500, unit="g")
...
def test_value_at_inx_2():
    r = parse_input(data=tup)
    a = AmountTuple(amount=1, quantity=None, unit=None)
